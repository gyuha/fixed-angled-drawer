...
```
파라미터 수정 → freecadcmd pen_holder.py → 검증 20항목 → VERIFICATION PASS → STL·FCStd 출력
                                              ↓ 실패
                                        FAIL 항목 출력 (출력물 사용 금지)
```

검증 항목은 유효 솔리드(6종), 바운딩박스(6종), 마감 모듈 높이, 마감 모듈 바닥 개방·천장 일체, 나사·스냅 정합, **체결 가능 상판 두께**, 앞턱 자기지지 각도, 앞면 필렛, 타공 배치까지 20가지입니다.
